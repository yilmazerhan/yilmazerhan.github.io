...
import {
  ...
  GanttChartSquare, Activity, CalendarDays, CalendarRange, Database, Megaphone, Layers, ListChecks, BookOpen, Rocket,
} from 'lucide-react'
...
// Grouped by content + access level: everyday work, planning/scheduling,
// reference resources, then admin-only people/team management and system config.
const NAV_GROUPS: NavGroup[] = [
  {
    ...
    items: [
      ...
      { to: '/team-tasks', icon: ListChecks, labelKey: 'nav.team_tasks' },
    ],
  },
  {
    ...
    items: [
      ...
      { to: '/releases', icon: Rocket, labelKey: 'nav.releases' },
  ...
  {
    labelKey: 'nav.group_resources',
    items: [
      { to: '/responsibility', icon: BookOpen, labelKey: 'nav.responsibility' },
      ...
      <nav className="flex-1 overflow-y-auto py-4 px-2 space-y-4">
        {NAV_GROUPS.map((group) => {
          const visibleItems = group.items.filter(
            ({ requiredRole }) => !requiredRole || !user || requiredRole.includes(user.role)
          )
          if (visibleItems.length === 0) return null
          return (
            <div key={group.labelKey}>
              <p className="px-3 pb-1 text-[11px] font-semibold uppercase tracking-wider text-gray-400 dark:text-gray-600">
                {t(group.labelKey)}
              </p>
